...
Em seguida nós simulamos a interação de uma pessoa com nossa página. Onde, naquele momento ela clica no botão.
````JS
render(<App />);
const button = screen.getByRole('button');
userEvent.click(button);
...
Agora estamos recuperando um elemento `<h2>` utilizando também o seletor *getByRole* e testando se este elemento é renderizado após o click e se o mesmo elemento contém em seu texto a palavra "Salve".
 ...
 No momento dos exercícios poderemos por em prática e absover ainda mais os conceitos vistos até aqui. Mas antes, que tal um pequeno quizz para ajudar a fixar o conteúdo antes de exercitar ?
